...
def generate_launch_description():
    ...
    slam_node = Node(
        ...
        respawn_delay=2.0,
        remappings=[
        ('/camera/rgb/image_color', '/camera/front_camera/image_raw'),
        ('/imu', '/imu/data'), 
        ],
        output='screen',
        parameters=[
    ...
    # Map -> Odom TF Bridge Node
    map_odom_bridge_node = Node(
        package='orbslam3_ros2',
        executable='map_odom_bridge.py',
    ...
    octomap_server_node = ExecuteProcess(
        cmd=['ros2', 'run', 'octomap_server', 'octomap_server_node', '--ros-args',
             '--remap', 'cloud_in:=/slam/pointcloud',
             '-p', 'use_sim_time:=true'],
