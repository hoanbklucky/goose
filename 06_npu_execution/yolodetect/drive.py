...
# --- CONFIGURATION ---
MODEL_PATH = '/home/radxa/ros2_ws/src/goose_ros2/goose_ros2/self_driving_best_rknn_model'
CAMERA_WIDTH = 640
CAMERA_HEIGHT = 480
...
# --- TUNING ---
ROI_VERTICAL_CUTOFF = 0.65 #0.45 # Was 0.65. Lowering allows seeing the line further ahead to start turning earlier
Kp = 0.0009  ##0.0009  #0.0010       0.0006        # Was 0.0005. Increased to allow stronger steering response to line error
Kd = 0.0016  ##0.0016  #0.0025`       0.001
BASE_SPEED = 0.18 #0.07       
...
# MOTOR PHYSICS
MIN_MOTOR_POWER = 0.1  
MAX_STEER = 0.85 #0.85 # Was 0.5. Increased to allow sharper turns at corners

# --- GLOBAL VARIABLES FOR STREAMING ---
output_frame = None
lock = threading.Lock()
run_flag = True
keyboard_drive_enabled = False
...
def check_keyboard_input():
    """Check if 'w' key is pressed. Returns True if 'w' is detected, False otherwise."""
...
def robot_control_loop():
    ...
    print("\n--- ROBOT STARTED ---")
    print("Press 'W' to toggle driving ON/OFF")

    try:
        # Set terminal to raw mode to detect key presses
        ...
        while run_flag:
            ...
            # --- DIGITAL ZOOM ---
            # Increase this factor to zoom in more (e.g., 1.5 for 1.5x, 2.0 for 2x)
            # zoom_factor = 1.5 
            ...
            #     # Calculate the cropped boundaries (center crop)
            #     top = (h - new_h) // 2
            #     left = (w - new_w) // 2
                
            #     # Crop and resize back to original resolution (640x480)
            ...
            # 4. Check Keyboard Input ('W' key toggles driving)
            if check_keyboard_input():
                keyboard_drive_enabled = not keyboard_drive_enabled
                time.sleep(0.2)  # Debounce to prevent rapid toggling
            
            # 5. Apply Driving Only if Toggle is ON
            if keyboard_drive_enabled:
		...
		f"best_y_x: {best_y_x}",
                ...
                f"Keyboard Drive: {'ON' if keyboard_drive_enabled else 'OFF'}",
            ]
            x0, y0 = 10, 25
